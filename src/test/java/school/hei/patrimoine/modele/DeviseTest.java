...
class DeviseTest {
  ...
  void testValeurEnAriaryForPastDate() {
    ...
    var expectedValue =
        (int)
            (devise.valeurEnAriary()
                + devise.valeurEnAriary()
                    * (devise.tauxDappréciationAnnuel() / 365)
                    * DAYS.between(pastDate, LocalDate.now()));
  ...
  void testValeurEnAriaryForFuturDate() {
    ...
    var valeurAjouteeJournaliere =
        devise.valeurEnAriary() * (devise.tauxDappréciationAnnuel() / 365.);
    var expectedValue =
        (int)
            (devise.valeurEnAriary()
                + valeurAjouteeJournaliere * (DAYS.between(futurDate, LocalDate.now())));
