...
class PossessionTest {
  ...
  void testGetValeurComptable() {
    ...
    var valeurAjouteeJournaliere =
        deviseUSD.valeurEnAriary() * (deviseUSD.tauxDappréciationAnnuel() / 365.);
    var conversionRate =
        (int)
            (deviseUSD.valeurEnAriary()
                + valeurAjouteeJournaliere * (DAYS.between(now, LocalDate.now())));
    int expectedValeur =
        (int) (argent.getValeurComptable() * argent.devise.valeurEnAriary(now) / conversionRate);
  ...
  void testProjectionFuture() {
    ...
    assertEquals(
        futureDate, projected.t); // Assuming the `projectionFuture` method updates the date
  }
